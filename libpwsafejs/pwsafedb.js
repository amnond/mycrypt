...
PWSafeDB.decryptFromUrl = function(url, passphrase, options) {
    if (options === undefined) {
        options = {};
    }

    if (!('on_complete' in options)) {
        options.on_complete = function() {}
    }
    if (!('percent_res' in options)) {
        options.percent_res = 5;
    }
            ...
            success: function(dataview) {
                if (useWebWorker) {
                    ...
                    worker.onmessage = function(event) {
                        ...
                        options.on_complete(result); return;
                    };
                    ...
                    try {
                        new PWSafeDB().decrypt(dataview.buffer, passphrase, options);
                    } catch (e) {
                        options.on_complete(e); return;
                    }
                }
            },
            error: function(jqXHR, textStatus) {
                options.on_complete(new Error("AJAX error. Status: "+textStatus)); return;
            }
        });
    } else { // node js
        fs.readFile(url, function(err, buffer) {
            if (err) {
                options.on_complete(err);
                return;
            }
            new PWSafeDB().decrypt(buffer, passphrase, options);
        });
    }
};
...
decrypt: function(buffer, passphrase, options) {
    if (!('on_complete' in options)) {
        options.on_complete = function() {}
    }
    var strictFieldTypeCheck = 'strictFieldTypeCheck' in options ? options.strictFieldTypeCheck : false;
    ...
    this._chunkWork(function() {
        ...
        var fieldView = this._decryptFields(keys, options);

        this._chunkWork(function() {

            this._readAllRecords(fieldView, strictFieldTypeCheck);
            ...
            this._chunkWork(function() {
                ...
                this._verifyHMAC(keys, (function(pdb) { return function(matched) {
                    ...
                    if (!matched) {
                        options.on_complete(new Error("HMAC didn't match -- something may be corrupted"));
                    } else {
                        options.on_complete(pdb); return;
                    }
                }; })(this));
...
_decryptFields: function(keys, options) {
    ...
    return this._newjDataView(TwoFish.decrypt(this._view, numFieldBlocks, keys.K, true, options));
},
...
_verifyHMAC: function(keys, callback) {
    ...
    if (PWSafeDB.isWebWorker) {
        ...
        console.log('decrypt: hashbytes', this._hashBytes)
    ...
    this.records = (function() {
        ...
        while (fieldView.tell() < fieldView.byteLength) {
            ...
            if (field.type === 0 && field.bytes.length === 0) {
                console.warn('empty field of type zero');
                continue; // <---
            }
        ...
        console.log("records after decrypt:", records)
        return records;
    }).call(this);
...
encrypt: function(passphrase, options) {
    var iterations = PWSafeDB.MIN_HASH_ITERATIONS;;
    if ('iterations' in options) {
        iterations = options.iterations;
        if (iterations < PWSafeDB.MIN_HASH_ITERATIONS) {
    ...
    view.writeBytes(TwoFish.encrypt(this._newjDataView(IV.concat(fieldsBuffer)), (fieldsBuffer.length / PWSafeDB.BLOCK_SIZE)+1, keys.K, true, options));

    // trailing data
    view.writeBinaryString('PWS3-EOFPWS3-EOF');
    console.log("encrypt: writing hashbytes", this._hashBytes);
...
if (PWSafeDB.isWebWorker) {
    ...
    onmessage = function(event) {
        ...
        try {
            new PWSafeDB().decrypt(data.buffer, data.passphrase, data.options);
        } catch (e) {
            callback(e); return;
        }
    };
