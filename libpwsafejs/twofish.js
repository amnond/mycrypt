...
exports.decrypt = function(dataView, blockCount, key, cbcMode, options) {
    var first_blockcount = blockCount;
    if (typeof(options) !== 'object') {
      options = {}
    }

    if (!('on_progress' in options) || typeof(options.on_progress)!='function') {
      options.on_progress = function() {};
    }
    if (!('percent_res' in options) || typeof(options.percent_res)!='number') {
    ...
    while (blockCount--) {
        ...
        var percent = parseInt(0.5 + 100.0 * (first_blockcount - blockCount - 1) / first_blockcount)
        if (percent % options.percent_res == 0 || percent == 100) {
...
exports.encrypt = function(dataView, blockCount, key, cbcMode, options) {
    var first_blockcount = blockCount;
    if (typeof(options) !== 'object') {
      options = {}
    }
